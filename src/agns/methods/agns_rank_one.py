...
from agns.methods.gns_rank_one import _make_wsm_solve
...
def agns_rank_one(
    ...
    *,
    ...
    """Run AGNS with a selectable dense/rank-one inverse backend.

    See :func:`agns.methods.agns.agns` for the shared parameters; ``invert_backend``
    selects ``"cho"`` (dense) or ``"wsm"`` (rank-one Sherman-Morrison).
    ...
    for k in range(n_iters + 1):
        ...
        if invert_backend == "cho":
            ...
            solve = _make_wsm_solve(approx_oracle, y_k, g_y, Binv_eff)
        else:
