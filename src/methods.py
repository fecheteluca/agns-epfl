...
def accelerated_grad_norm_smooth(
    ...
    """
    Run Accelerated Gradient-Regularized Newton Method (AGNS).
    Extends Algorithm 1 with Nesterov momentum and decoupled evaluation sequences.
    """
    oracle = OracleCallsCounter(oracle)
    start_timestamp = datetime.now()
    ...
    # Initialization
    x_k = np.copy(x_0)
    x_prev = np.copy(x_0) # Track previous point for momentum
    f_k = oracle.func(x_k)
    g_k = oracle.grad(x_k)
    g_k_norm = dual_norm_sqr(g_k) ** 0.5
    gamma_k = gamma_0
    ...
    for k in range(n_iters + 1):
        ...
        # 1. Momentum Extrapolation (Fixed Nesterov Sequence)
        beta_k = k / (k + 3.0)
        y_k = x_k + beta_k * (x_k - x_prev)

        # 2. Oracle Evaluation at Lookahead
        g_y = oracle.grad(y_k)
        ...
        for i in range(adaptive_search_max_iter + 1):
            ...
            try:
                # Regularized Newton step from y_k
            ...
            # AGNS Progress Condition (Checked against the lookahead value f_y)
        ...
        # 5. Gradient Restart Check
        # Check if momentum moved us in an adversarial direction relative to the new gradient
        if g_trial.dot(y_k - x_k) > 0:
            x_prev = x_trial.copy() # Reset momentum state
        else:
            x_prev = x_k.copy()     # Standard update
        ...
        for i in range(adaptive_search_max_iter + 1):
            ...
            try:
                if invert_backend == "cho":
                    ...
                    # Evaluate components at the chosen anchor point!
